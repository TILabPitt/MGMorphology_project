...
%INPUT: 3D array of binary image of one cell at a time
    ...
    nm=bwconncomp(nucmask);%use connected components to identify each spot
    if isempty(nm.PixelIdxList)
        SmoothEx = imbinarize(imgaussfilt(CellImg,2));
        nucmask=imerode(SmoothEx,se);
        nm=bwconncomp(nucmask);
        if isempty(nm.PixelIdxList)
            se = strel('diamond',2);
            nucmask=imerode(CellImg,se);
            nm=bwconncomp(nucmask);
            if isempty(nm.PixelIdxList)
                se = strel('diamond',2);
                SmoothEx = imbinarize(imgaussfilt(CellImg,2));
    ...
    s = size(CellImg);
    ex1=zeros(s(1),s(2));%Create blank image of correct size

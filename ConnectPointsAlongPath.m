...
%Find the shortest path connecting the centroid and the endpoints.
...
%value 1 in the original image. 
    ...
    D = repmat(init(1+InputImg),[1 1 2]); %repmat repeats copies of a matrix. This duplicates the BoundedSkel image. It also multiplies it by init, so all 0s become Nan and all 1s become Inf
    D(StartPt(1),StartPt(2),1) = 0; %set the initial and end points to 0.
    D(EndPt(1),EndPt(2),2) = 0;
    ...
    while isinf(D(EndPt(1),EndPt(2)))&nnz(mask),% continue stepping while the endpoint (but in the first image) is inf. Once the path is connected, this value will be overwritten to 1, meaning the loop is complete. nnz = number of nonzero elements. If the path is not completed, but there are no remaining connections, the mask will only have 0s. Both of these conditions must be 1 (true) to continue.
        n = n+1;
        mask = convn(mask,ones([3 3]),'same')&isinf(D); %Convolves with cube of ones to see attached pixels?
        D(mask) = n;
    end
    if isinf(D(EndPt(1),EndPt(2))) %If the above loop ended, it's complete or there's no path. If it ended, but the first condition is still true (the endpoint is still Inf), then there must not have been a connecting path.
        error('no path found');
    else
        mask = sum(D,3)==n; % points within minimum-length path(s). sums the two 3D matrices. Only those paths ==n are the shortest and the rest are ignored.

...
[mr,mc]=ind2sub(si,pxlist); %Convert indexed values to x y z coordinates (row, column, plane)
distpoint = [mr mc];% Concatenate coordinates into single matrix
    
epind = find( (distpoint(:,1) == ep(1)) & (distpoint(:,2) == ep(2))); %Get row index of endpoint in distpoint list
...
ii = 1; 
    while ii<length(distpoint) && sum(distpoint(ii,:)==stp)~=2; %Run loop until either hit the last row of distpoint, or we hit the centroid
